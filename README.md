# eneba-03-internal-linking-engine
...
(generate the pages) and [eneba-02-crawl-redirect-mapper](https://github.com/Bristotle/eneba-02-crawl-redirect-mapper)
...
usually left to a template rule -- breadcrumbs plus "related products" -- and
that rule links by category, not by relevance. The result is a catalogue where
...
The tool reports these separately, with the hub that should own each topic and a
suggested anchor, rather than filing them under "still orphaned" and stopping.
...
out/report.md              before/after summary, PageRank movers, full change list
out/graph.json             every node with inlinks and PageRank before/after,
...
Standard library only. Point `--pages` at a real inventory export to run it
against a live site.

## Result on the bundled fixture

12 pages, 15 existing internal links. The engine finds 13 unlinked mentions and
proposes all 13, taking the graph to 28 links. The two most under-linked product
pages gain the most: Witcher 3 +113 percent internal PageRank, Elden Ring +107
percent, funded largely by the RPG hub's share falling 11 percent. That
redistribution is the point. Hub pages accumulate rank by default and have the
least need for it.
